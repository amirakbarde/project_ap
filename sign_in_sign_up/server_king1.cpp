...
void server_king1::getting(QString u,QString p){
    username =u;
    path=p;
}
...
void server_king1::rounds(){
    ...
    my_socket->write("0");
    my_socket->waitForReadyRead(-1);
    QString e_user = my_socket->readAll();
    my_socket->write("0");
    my_socket->waitForReadyRead(-1);
    QString e_path = my_socket->readAll();
    if(enemy_score.toInt()>score){
        ...
        if (!file.open(QIODevice::ReadWrite | QIODevice::Text))
        {
            ...
            fileContents[8] =enemy_score+"-"+QString::number(score) ;
            file.resize(0);
        ...
        if (!file2.open(QIODevice::ReadWrite | QIODevice::Text))
        {
            ...
            fileContents[8] =QString::number(score)+ "-" +enemy_score;
            file2.resize(0);
